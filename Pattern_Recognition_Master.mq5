                ...
                const int &spread[])
{
    ...
    if (prevtime == Time[0]) return rates_total;
    prevtime = Time[0];

    for (int shift = 0; shift < rates_total - 9; shift++)
    {
        int CumOffset = 0;
        bool setalert = false;
...
{
    return ObjectPrefix + aName + TimeToString(timeshift);
}
